...
class _MnasNet(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        ...
        # building first two layer
        self.layer0 = nn.Sequential(*[Conv_3x3(3, input_channel, 2), SepConv_3x3(input_channel, 16)])
        input_channel = 16
        '''
        self.layer1 = []
        self.layer2 = []
        ...
        self.layer5 = []
        self.layer6 = []'''
        self.layers = nn.ModuleList()  # [self.layer1, self.layer2, self.layer3, self.layer4, self.layer5, self.layer6]

        # building inverted residual blocks (MBConv)
        for i, (t, c, n, s, k) in enumerate(self.interverted_residual_setting):
            output_channel = int(c * width_mult)
            layer = []
            for i in range(n):
                if i == 0:
                    layer.append(InvertedResidual(input_channel, output_channel, s, t, k))
                else:
                    layer.append(InvertedResidual(input_channel, output_channel, 1, t, k))
                input_channel = output_channel
    ...
    def forward(self, x):
        ...
        x = self.layers[5](x)
        x_highlevel = x
        return x_lowlevel, x_midlevel, x_highlevel
