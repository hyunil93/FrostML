...
class PoolMixer(Module):
    ...
    def __init__(self, kernel_size: int = 3):
        ...
        self.pool = AvgPool2d(kernel_size=kernel_size, stride=1, padding=kernel_size // 2, count_include_pad=False)
...
class EfficientFormer(Module):
    ...
    def __init__(self, embed_dims, layers, num_classes: int = 1000, device=None, dtype=None) -> None:
        factory_kwargs = dict(device=device, dtype=dtype)
        super(EfficientFormer, self).__init__()
        self.conv1 = Conv2d(3, int(embed_dims[0] / 2), 3, 2, 1, bias=False, **factory_kwargs)
        self.bn1 = BatchNorm2d(int(embed_dims[0] / 2), **factory_kwargs)
        self.conv2 = Conv2d(int(embed_dims[0] / 2), embed_dims[0], 3, 2, 1, bias=False, **factory_kwargs)
    ...
    @staticmethod
    def _make_layer(blk: Type[Union[MetaBlock4D, MetaBlock3D]], planes: int, next_planes: int,
                    blocks: int, stride: int = 1, device=None, dtype=None) -> Sequential:
        factory_kwargs = dict(device=device, dtype=dtype)
...
def _efficientformer(pretrained: bool, embed_dim: List[int], layers: List[int], **kwargs: Any):
